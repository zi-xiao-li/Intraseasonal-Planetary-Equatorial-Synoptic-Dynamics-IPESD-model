...
|------------------|-------------|
| `src/IPESD-syn.m` | Defines synoptic-scale heating and computes velocity, temperature, and pressure perturbations. |
| `src/IPESD-main.m` | Solves the planetary-scale QLELWE system using Hermite spectral decomposition and generates circulation responses. |
| `data/` | Stores intermediate and output matrices (`l`, `v`, `r`) generated by the model. |
